...
from AR_sudoku import AR_Sudoku
...
class SudokuGUI:
    ...
    def __init__(self, matrix: np.ndarray, box_rows: int = 3, box_cols: int = 3):
        ...
        self.button_solve = Button(488, 510, 250, 60, (200, 200, 200), "Solve")
        # play game button
            ...
            if rectangles:
                ...
                if not valid_sudoku:
                    ...
                    if valid_frame_count >= 7:
                        ...
                        sudoku = AR_Sudoku(''.join(map(str, board.astype(np.int))))
                        solution = sudoku.solve()
                        print("Solution : ", ARsolution)
    ...
    def main_menu(self):
        ...
        label = font.render('Sudoku Game', 1, (0, 0, 0))
        # display the heading
        self.window.blit(label,
                         ((SCREEN_WIDTH - label.get_width()) / 2,
